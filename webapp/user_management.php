...
/* Get All the Users */
$app->get("/getAllTest", function () use ($app) {
    $db = new DbHandler();
    $response = array();
    //$request = json_decode($app->request->getBody());
    //var_dump($request);
    ...
    $sql = "SELECT * FROM test WHERE address LIKE '%$keyword%'";
    $row = $db->conn->query($sql) or die($this->mysqli->error.__LINE__);
    if ($row->num_rows > 0) {
        $result = array();
        while ($r = $row->fetch_assoc()) {
...
$app->post('/userLogin', function () use ($app) {
    ...
    $sql = "SELECT id FROM user WHERE username='$request->username' AND password='$request->password'";
    $row = $db->conn->query($sql) or die($this->mysqli->error.__LINE__);
    if ($row->num_rows > 0) {
        $result = array();
        while ($r = $row->fetch_assoc()) {
        ...
        $response["message"] = "Username or id is not valid.";
        $response["cause"] = "";
        $response["response"] = [];
        $response["data"] = "0";
        echoResponse(200, $response);
